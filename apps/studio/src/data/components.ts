...
export const getPopularComponents = (limit = 5): ComponentMetadata[] => {
  return componentRegistry
    .sort((a, b) => b.popularity - a.popularity)
    .slice(0, limit);
};

export const getNewComponents = (limit = 5): ComponentMetadata[] => {
  return componentRegistry
    .sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime())
    .slice(0, limit);
};

export const getPremiumComponents = (): ComponentMetadata[] => {
  return componentRegistry.filter(component => component.premium);
};
...
export const getComponentsByComplexity = (complexity: 'beginner' | 'intermediate' | 'advanced'): ComponentMetadata[] => {
  return componentRegistry.filter(component => component.complexity === complexity);
};

export const getComponentsByFeature = (feature: keyof ComponentFeatures): ComponentMetadata[] => {
  return componentRegistry.filter(component => component.features[feature]);
};

// Statistics helpers
export const getComponentStats = () => {
  const total = componentRegistry.length;
  const categories = new Set(componentRegistry.map(c => c.category)).size;
  const premium = componentRegistry.filter(c => c.premium).length;
  const popular = componentRegistry.filter(c => c.popularity > 90).length;
  const accessible = componentRegistry.filter(c => c.features.accessible).length;
  const responsive = componentRegistry.filter(c => c.features.responsive).length;
  
  const avgPopularity = componentRegistry.reduce((sum, c) => sum + c.popularity, 0) / total;
  const avgBundleSize = componentRegistry.reduce((sum, c) => sum + c.performance.bundleSize, 0) / total;
  ...
  return {
    ...
    complexityBreakdown: {
      beginner: componentRegistry.filter(c => c.complexity === 'beginner').length,
      intermediate: componentRegistry.filter(c => c.complexity === 'intermediate').length,
      advanced: componentRegistry.filter(c => c.complexity === 'advanced').length
    }
  };
};